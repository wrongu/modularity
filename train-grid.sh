...
parallel --linebuffer --tag --jobs=24 $ENV $PYTHON -m train --dataset={1} --task={2} --save-dir=logs --device=auto --devices=$CUDA_DEVICES --l2={3} --run={4} \
  ::: ${DATASETS[@]} ::: ${TASKS[@]} ::: ${L2VALS[@]} ::: ${RUNS[@]}

parallel --linebuffer --tag --jobs=24 $ENV $PYTHON -m train --dataset={1} --task={2} --save-dir=logs --device=auto --devices=$CUDA_DEVICES --l1={3} --run={4} \
  ::: ${DATASETS[@]} ::: ${TASKS[@]} ::: ${L1VALS[@]} ::: ${RUNS[@]}

parallel --linebuffer --tag --jobs=24 $ENV $PYTHON -m train --dataset={1} --task={2} --save-dir=logs --device=auto --devices=$CUDA_DEVICES --drop={3} --run={4} \
